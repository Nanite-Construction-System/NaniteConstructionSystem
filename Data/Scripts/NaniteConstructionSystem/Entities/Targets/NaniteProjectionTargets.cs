        ...
        private void ProcessBuildBlock(IMySlimBlock block)
        {
            try {
                ...
                foreach(var item in NaniteConstructionManager.ProjectorBlocks)
                {
                    ...
                    // The Multigrid Projector expects the subgrid index to be passed in the builtBy argument,
                    // which is redundant (always the same as the owner) in the vanilla game
                    var ownerId = m_constructionBlock.ConstructionBlock.OwnerId;
                    var builtBy = ProjectorIntegration.MgpAgent.Available ? (long) subgridIndex : ownerId;

                    projector.Build(block, ownerId, m_constructionBlock.ConstructionBlock.EntityId, false, builtBy);
                    break;
                }
            } catch(Exception exc) {
